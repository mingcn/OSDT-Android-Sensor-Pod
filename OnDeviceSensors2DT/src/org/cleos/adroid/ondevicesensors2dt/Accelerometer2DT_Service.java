...
public class Accelerometer2DT_Service extends Service {

	private String TAG = getClass().getSimpleName();
	private String name = "Accelerometer";
	private SensorManager mSM;
	private Sensor sensor;
	private Accelerometer2DT_Thread thread;
	private String ipp;
	...
	private void spawThread() {
		...
		String[] chNames = { "x", "y", "z", "linear_acceleration_X",
				"linear_acceleration_Y", "linear_acceleration_Z" };

		String[] dTypes = { "float64", "float64", "float64", "float64",
				"float64", "float64" };

		String[] units = { "m/s^2", "m/s^2", "m/s^2", "m/s^2", "m/s^2", "m/s^2" };
		...
		thread = new Accelerometer2DT_Thread("Accelerometer", this, ipp, mSM,
				sensor);
	...
	private Sensor sSensor(SensorManager sm) {
		if (sm.getDefaultSensor(Sensor.TYPE_ACCELEROMETER) != null) {
			return sm.getDefaultSensor(Sensor.TYPE_ACCELEROMETER);
		} else {
			String msg = "The Accelerometer is no pressent in this device!";
			Log.e(TAG, msg);
			msgToast(msg);
			return null;
		}
