...
public class DTSrcInserter {
	private String TAG = "DTSrcInserter";
		...
		public void run() {
			...
			if (chNames != null) {
				...
				try {
					...
					Log.e(TAG,
							"Error in parsing the data when inserting the data to RBNB server. "
									+ ne.getMessage());
					log.writelnT("Error in parsing the data when inserting the data to RBNB server.  "
							+ ne.getMessage());
	...
	private void flushToServer() throws SAPIException {
		...
		Log.d(this.TAG, "Flushed " + flushedChCount
				+ " many channels to the DT server");
	}
