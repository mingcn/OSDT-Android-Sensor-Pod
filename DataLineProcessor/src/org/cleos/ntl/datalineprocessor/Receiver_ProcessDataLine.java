 ...
 * If the service is unlock, it will start a new thread to call the DLP and 
...
public class Receiver_ProcessDataLine extends BroadcastReceiver {
	...
	private synchronized void receivedCommand(Bundle extras) {
		if (!Utils.isLockService(this.context, flagFile)) {
			...
			Log.i(TAG,
					"The DLP service is LOCKED!!! The actual number of request is: "
							+ numLocks);
			if (numLocks >= 153) {
				Log.i(TAG,
						"The DLP service looks to be locked. Unlocking DLP and DT.");
				// unlock DLP
				Utils.lockService(context, flagFile, false);
				// unlock DT
				SendBroadcast.unlockDTservice(context);
				Utils.setZeroLockCounter(context);
			}
		}
